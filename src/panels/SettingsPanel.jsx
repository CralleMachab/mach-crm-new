...
export default function SettingsPanel({
  ...
  const [importInfo, setImportInfo] = useState("");
    ...
    const [openArchivedActivity, setOpenArchivedActivity] = useState(null);
    ...
    {archivedActivities.map((a) => (
      ...
      >
        ...
        <button
          ...
          onClick={() => setOpenArchivedActivity(a)}
          type="button"
        >
          Öppna
        </button>
    ...
    <div className="w-full max-w-xl rounded-2xl bg-white p-4 shadow-lg">
      <div className="flex justify-between items-start">
        <div>
          <h3 className="font-semibold text-lg">
            {openArchivedActivity.title || "Aktivitet"}
          </h3>
          <div className="text-sm text-gray-600">
            Skapad: {openArchivedActivity.createdAt?.slice(0, 10) || "—"}
          </div>
          <div className="text-sm text-gray-600">
            Arkiverad: {openArchivedActivity.deletedAt?.slice(0, 10) || "—"}
          </div>
        </div>

        <button
          onClick={() => setOpenArchivedActivity(null)}
      ...
      <div className="mt-4 flex justify-end gap-2">
        <button
          ...
          onClick={() => {
            restoreActivity(openArchivedActivity.id);
            setOpenArchivedActivity(null);
          }}
        >
          Återställ
        </button>
        ...
        <button
          className="px-3 py-2 rounded bg-rose-700 text-white text-sm"
          onClick={() => {
            if (!window.confirm("Ta bort permanent? Detta går inte att ångra.")) return;
            deleteActivityForever(openArchivedActivity.id);
            setOpenArchivedActivity(null);
          }}
        >
          Ta bort permanent
    ...
    <div className="w-full max-w-xl rounded-2xl bg-white p-4 shadow-lg">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {openArchivedActivity.dueDate && (
            <div className="text-sm text-gray-600">
              Nästa datum: {openArchivedActivity.dueDate}
            </div>
          )}
        </div>
      ...
      {/* Detaljer (visa det viktigaste utan att göra det komplicerat) */}
      <div className="mt-3 space-y-2 text-sm">
        {openArchivedActivity.customerName && (
          <div><span className="font-semibold">Kund:</span> {openArchivedActivity.customerName}</div>
        )}
        {openArchivedActivity.contactName && (
          <div><span className="font-semibold">Kontakt:</span> {openArchivedActivity.contactName}</div>
        )}
        {openArchivedActivity.notes && (
      ...
      <div className="mt-4 flex flex-wrap gap-2 justify-end">
        ...
        <button
          className="text-sm px-3 py-2 rounded bg-rose-700 text-white"
          onClick={() => {
            const ok = window.confirm("Ta bort PERMANENT? Detta går inte att ångra.");
            if (!ok) return;
            deleteActivityForever(openArchivedActivity.id);
            setOpenArchivedActivity(null);
          }}
