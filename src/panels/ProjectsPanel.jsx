...
import PlannerHome from "../planner/src/pages/Home.jsx";
...
const createEmptyPlanner = () => ({
  ...
  const normalizeNotes = (p) => {
    const raw = Array.isArray(p?.notes) ? p.notes : null;
    if (raw && raw.length) {
      return raw
        .map((n) => ({
          id: n?.id || mkId(),
    ...
    const legacyText = p?.note || "";
    if (legacyText && typeof legacyText === "string") {
      const legacyDate = (p?.updatedAt || p?.createdAt || "").slice(0, 10);
...
export default function ProjectsPanel({
  ...
  useEffect(() => {
    ...
    setDraft({
      id: p.id,
      projectNumber: (p.projectNumber ?? p.number ?? p.projectNo ?? p.internalId ?? null),
      name: p.name || "",
      ...
      kind: p.kind || "",
      planner: p.planner || createEmptyPlanner(),
    });
    setState((s) => ({
      ...s,
    ...
    setState((s) => ({
      ...
      projects: (s.projects || []).map((p) =>
        p.id === draft.id
          ? {
              ...p,
              projectNumber: (draft.projectNumber ?? p.projectNumber ?? p.number ?? p.projectNo ?? p.internalId ?? null),
              name: draft.name || "",
              ...
              note: draft.note || "",
              planner: draft.planner || p.planner || createEmptyPlanner(),
              files,
  ...
  const emailProject = () => {
    ...
    if (draft.projectNumber) {
      lines.push(`Projektnr: ${draft.projectNumber}`);
    }
    if (draft.status) {
...
<body>
  <div class="page">
    ...
    <div class="meta-grid">
      ...
      ${
        draft.projectNumber
          ? `<div><span class="label">Projektnr:</span> <span class="value">${draft.projectNumber}</span></div>`
          : ""
      }
      <div>
  ...
  return (
    <div className="bg-white rounded-2xl shadow p-4">
      ...
      <ul className="divide-y">
        {list.map((p) => {
          const projNum = (p.projectNumber ?? p.number ?? p.projectNo ?? p.internalId) ?? null;
          const wonOffer =
          ...
          >
                ...
                <div className="text-right">
                  ...
                  <input
                    ...
                    style={{
                      ...
                      width: `${Math.max(String(draft.projectNumber ?? "").length + 3, 9)}ch`,
                    }}
                  />
                </div>
                <button
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <div className="w-full border rounded px-3 py-2 min-h-[80px] bg-gray-50 flex items-center justify-between gap-3">
                    ...
                    onClick={() => {
                      if (!draft?.planner) {
                        const ok = window.confirm(
                          "Detta projekt har ingen tidplan ännu. Vill du skapa en ny?"
                        );
                        if (!ok) return;
  ...
  >
    <div
      className="bg-white w-screen h-screen overflow-auto relative"
      onClick={(e) => e.stopPropagation()}
    >
      <button
        type="button"
