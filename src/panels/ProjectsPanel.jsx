...
export default function OffersPanel({ offers = [], entities = [], setState }) {
  ...
  // Öppna direkt om _shouldOpen är satt (nyss skapad offert)
  useEffect(() => {
    ...
    setDraft({
      ...
      filesList: flattenFiles(o.files),
  ...
  const list = useMemo(() => {
    ...
    if (q.trim()) {
      ...
      arr = arr.filter((o) => {
        ...
        return (
          ...
          (o.offerNumber || "").toString().includes(s) ||
          cName.includes(s)
        );
      });
    }
    ...
    if (statusFilter !== "all") {
      arr = arr.filter((o) => (o.status || "utkast") === statusFilter);
    }

    arr.sort((a, b) => (b.createdAt || "").localeCompare(a.createdAt || ""));
    return arr;
  }, [offers, q, statusFilter, customers]);
        ...
        o.id === draft.id
          ? {
              ...
              supplierIds: Array.isArray(draft.supplierIds)
                ...
                : [],
              offerNumber: draft.offerNumber || o.offerNumber || "",
  ...
  const softDelete = (o) => {
    if (
      !window.confirm(
        "Ta bort denna offert? Den hamnar i Arkiv (kan tas bort permanent via Inställningar)."
      )
    )
      return;
    setState((s) => ({
  ...
  const createProjectFromOffer = () => {
    if (!draft) return;
    setState((s) => {
      const offer = (s.offers || []).find((o) => o.id === draft.id);
      if (!offer) return s;

      // Kontroll: finns redan projekt från denna offert?
      const existing = (s.projects || []).find(
        (p) => p.originatingOfferId === offer.id
      );
      if (existing) {
        alert("Det finns redan ett projekt skapat från denna offert.");
        return s;
      }
      ...
      const p = {
        ...
        status: "pågående",
        budget: Number(offer.value) || 0,
        startDate: "",
        endDate: "",
        note: offer.note || "",
        files: offer.files || { Ritningar: [], Offerter: [], Kalkyler: [], KMA: [] },
        projectNumber,
        internalId: projectNumber,
        originatingOfferId: offer.id,
        supplierIds: Array.isArray(offer.supplierIds)
          ? offer.supplierIds.slice()
          : [],
        kind: "",
      ...
      const nextOffers = (s.offers || []).map((o) =>
        o.id === offer.id
          ? {
              ...o,
              status: "vunnen", // markera som vunnen när projekt skapas
              updatedAt: new Date().toISOString(),
            }
          : o
      );
    ...
    alert("Projekt skapat från offerten. Gå till fliken Projekt för att se det.");
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow p-4">
      <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
        ...
        <div className="flex gap-2 flex-wrap items-center">
          <input
            ...
            placeholder="Sök på titel, offertnr, kund..."
            value={q}
      ...
      <ul className="divide-y">
        {list.map((o) => {
          const hasProject = false; // vi visar inte status här, men skulle kunna läsa från state via setState – enklare att hoppa.
          return (
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                >
                  ...
                  <div className="text-xs text-gray-500 flex items-center gap-2 flex-wrap">
                    <span>Kund: {customerName(o.customerId)}</span>
                    {o.value != null && (
                      <span>
                        {(Number(o.value) || 0).toLocaleString("sv-SE")} kr
                      </span>
                    )}
                    {o.nextActionDate && (
                      <span>Nästa åtgärd: {o.nextActionDate}</span>
                    )}
                  </div>
                </button>
                <div className="flex items-center gap-2 shrink-0">
                  <span className={offerStatusBadge(o.status)}>
                    {o.status || "utkast"}
                  </span>
                  {hasProject && (
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-sm font-medium">Offertnr</label>
                <input
                  className="w-full border rounded px-3 py-2"
                  value={draft.offerNumber || ""}
                  onChange={(e) =>
                    setDraft((d) => ({ ...d, offerNumber: e.target.value }))
                  }
                  placeholder="t.ex. 35001"
                />
              </div>
