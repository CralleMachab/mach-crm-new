// lib/storage.js
const STORAGE_KEY = "machcrm_data_v3_DEV";
...
export function upsertActivity(state, activity) {
  const arr = state.activities || (state.activities = []);
  const i = arr.findIndex(x => x.id === activity.id);
  if (i >= 0) arr[i] = activity; else arr.unshift(activity);
  saveState(state); return state;
}
